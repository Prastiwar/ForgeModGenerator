    ...
    public class McModInfoJsonConverter : JsonConverter<McModInfo>
    {
        ...
        public override void WriteJson(JsonWriter writer, McModInfo value, JsonSerializer serializer)
        {
            JObject jo = new JObject {
                ...
                { "authorList", JToken.FromObject(value.AuthorList, serializer) },
                { nameof(McModInfo.Screenshots).ToLower(), JToken.FromObject(value.Screenshots) },
                { nameof(McModInfo.Dependencies).ToLower(), JToken.FromObject(value.Dependencies) }
            };
            jo.WriteTo(writer);
        }
    }
